"""
ivpyode/__init__.py
A module containing extensions to the PyODE + iVisualPython Libraries.
...
class Mesh(ode.TriMeshData):
    ...
    def MakeMeshObject(self):
        ...
        #return ivisual.faces(pos=self._Verticies, colors=self._Colors, normals=self._Normals)
        return ivisual.convex(pos=self._verticies)
...
class GDMElement(DisplayElement):
    ...
    def DefineCylinder(self, ElementKey, Body, Density, Radius, Length):
        ...
        cyl = ivisual.cylinder(frame=Body._myFrame, pos=(0, 0, -Length / 2.0), axis=(0, 0, Length), radius=Radius)
    ...
    def DefineCylinderTotal(self, ElementKey, Body, TotalMass, Radius, Length):
    ...
    def DefineMeshTotal(self, ElementKey, Body, TotalMass, CenterOfMass, InertiaMatrix, Mesh):
        """Define this element as a mesh.  The Mesh must be a ivpyode.Mesh class object.  The
        cg and IntertiaMaxtrix are defined in world coordinates."""
        if self._hasGeom:
            self._geom = ode.GeomTriMesh(Mesh, None)

        DisplayElement.SetDisplayObject(self, Mesh.MakeMeshObject(frame=Body._myFrame))
...
class GDMFrameBody(odelib.BaseBody):
    ...
    def __init__(self, World):
        ...
        # Create a ivisual frame to hold display objects
        self._myFrame = ivisual.frame()
